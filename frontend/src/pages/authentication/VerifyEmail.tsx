      ...
      >
        ...
        <div className="md:w-[60%] p-6 md:p-8">
          ...
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-2 mb-6">
            <p className="text-gray-800 text-lg font-medium">Check your email</p>
...
const VerifyEmailForm = () => {
  ...
  const resend = () => {
    ...
    // Trigger resend flow here
    setResendSecondsLeft(RESEND_COOLDOWN_SECONDS);
  };

  return (
    <form onSubmit={onSubmit} className="space-y-6" onPaste={handlePaste}>
      ...
      <div className="flex gap-2 justify-center">
        {digits.map((digit, i) => (
          <input
            ...
            className="w-10 h-11 md:w-12 md:h-12 text-center text-base md:text-lg border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-sky-500 focus:border-transparent shadow-sm"
          />
        ))}
      </div>
      ...
      <div className="text-center text-xs text-gray-500 pt-1">Having trouble? Try resending the code.</div>
    </form>
  );
};
